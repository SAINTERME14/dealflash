...
const ADMIN_FLASH_STORAGE_KEY = "df_admin_flash_items";

export function getAdminFlashItems(): AdminFlashItem[] {
  try {
    const stored = localStorage.getItem(ADMIN_FLASH_STORAGE_KEY);
...
export function saveAdminFlashItems(items: AdminFlashItem[]): void {
  localStorage.setItem(ADMIN_FLASH_STORAGE_KEY, JSON.stringify(items));
}
