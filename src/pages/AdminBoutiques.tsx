...
export default function AdminBoutiques() {
  ...
  const filtered = useMemo(() => {
    return items.filter((b) => {
      ...
      const matchBadge = badgeFilter === "all" || (badgeFilter === "official" ? b.official : !b.official);
      ...
      <div ref={sectionRef} style={{ fontFamily: "Inter, system-ui, sans-serif", color: "white" }}>
        ...
        {selected.size > 0 && (
          <div style={{ display: "flex", gap: 8, marginBottom: 12, padding: "10px 14px", background: "#1a1a1a", border: "1px solid #FFD000", borderRadius: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => { persist(items.map((b) => (selected.has(b.id) ? { ...b, status: "active" } : b))); setSelected(new Set()); }} style={smallBtnStyle}>✅ Activer</button>
            <button onClick={() => { if (confirm(`Supprimer ${selected.size} boutique(s) ?`)) { persist(items.filter((b) => !selected.has(b.id))); setSelected(new Set()); } }} style={{ ...smallBtnStyle, background: "#7f1d1d", borderColor: "#ef4444", color: "#fca5a5" }}>🗑️ Supprimer la sélection</button>
          </div>
        )}
        ...
        <AdminPagination2
          ...
          onPageChange={(p) => { setCurrentPage(p); sectionRef.current?.scrollIntoView({ behavior: "smooth" }); }}
        />

        {modal.open && (
          <BoutiqueModal item={modal.item} onSave={handleSave} onClose={() => setModal({ open: false, item: null })} />
        )}
      </div>
    </AdminLayout>
  );
